...
########################################################## READ DATA ##########################################################
# Read in MAF file
MAF_file <- read.table("./0_data/PDX/AllSamples_Rem_HPV_Lymphoma.maf",header=TRUE,row.names = 1)
ClinicalInfo <- read.csv("./0_data/PDX/PDX_Rem_HPV_Lymphoma_Clinical_Information.csv", header = T)
...
########################################################## SET VARIABLES ##########################################################
data <- "./0_data/PDX/"
pipelines <- "./2_pipelines/MAFtools/PDX/Filtered_HPV_Lymphoma_VAF0.1/"
output <- "./3_output/MAFtools/PDX/Filtered_HPV_Lymphoma_VAF0.1/"
...
# Remove VAF <= 10%
maf_filtered <- subsetMaf(maf=maf_filtered, query="VAF > 0.1")
maf_filtered 
...
oncoplot(maf_filtered,top=10, titleText = "Altered in 59 (96.72%) of 61 PDX Samples.", # title took from normal oncoplot function, but added in PDX
         colors = onco_colours,
         drawColBar = FALSE, # Removes top TMB bar chart
         drawRowBar = FALSE) # Removes right % mutation bar chart
dev.off()
...
for (gene in genes) {
  tryCatch({ # Handles errors and loops through all genes
...
p_resp <- ggplot(tnm_df, aes(x = Trinucleotide, y = Frequency, fill = MutationContext)) +
  ...
  theme_bw() +
  labs(title = "PDX Average Trinucleotide Mutational Frequencies",
       x = "Trinucleotide",
       y = "Average Frequency") +
  theme(
