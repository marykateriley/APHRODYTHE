## Modified from ComplexHeatmap::Pheatmap
new_heatmap <- function (mat, color = colorRampPalette(rev(brewer.pal(n = 7,  
                         ...
                         annotation_show_legend = NULL, annotation_legend_param = NULL,  
                         ...
                         number_format = "%.2f", number_color = "grey30", fontsize_number = 0.8 * fontsize, gaps_row = NULL, gaps_col = NULL, labels_row = NULL,  
                         ...
                         legend_orientation = "vertical")
{
  ...
  # *** NEW: Apply the user's chosen orientation to the Z-score legend ***
  heatmap_legend_param$direction = legend_orientation
  ...
  if (!identical(annotation_col, NA)) {
    ...
    for (nm in colnames(annotation_col)) {
      if (nm %in% names(annotation_colors)) {
        if (!is.function(annotation_colors[[nm]]) &&  
            is.null(names(annotation_colors[[nm]])) &&  
    ...
    # choose show_legend spec: single logical or named logical per-annotation
    .show_legend_top <- if (is.null(annotation_show_legend)) annotation_legend else annotation_show_legend
    
    # choose legend params per-annotation: NULL or named list
    .anno_legend_param_top <- if (is.null(annotation_legend_param)) NULL else annotation_legend_param
    
    # *** NEW: Apply the user's chosen orientation to the annotation legend ***
    if (is.null(.anno_legend_param_top)) {
      .anno_legend_param_top = list(direction = legend_orientation)
    } else {
      .anno_legend_param_top$direction = legend_orientation
    }
    
    ht_param$top_annotation = HeatmapAnnotation(
      df = annotation_col[, ncol(annotation_col):1, drop = FALSE],
      col = annotation_colors,
      show_legend = .show_legend_top,
      show_annotation_name = annotation_names_col,
      gp = gpar(col = border_color),
      annotation_name_gp = gpar(fontsize = 10, fontface = "bold"),
      simple_anno_size = unit(10, "bigpts"),
      gap = unit(2, "bigpts"),
      annotation_legend_param = .anno_legend_param_top
    )
  }
  if (!identical(annotation_row, NA)) {
    ...
    .show_legend_left <- if (is.null(annotation_show_legend)) annotation_legend else annotation_show_legend
    .anno_legend_param_left <- if (is.null(annotation_legend_param)) NULL else annotation_legend_param
    
    # *** NEW: Apply the user's chosen orientation to the annotation legend ***
    if (is.null(.anno_legend_param_left)) {
      .anno_legend_param_left = list(direction = legend_orientation)
    } else {
      .anno_legend_param_left$direction = legend_orientation
    }
    
    ht_param$left_annotation = rowAnnotation(
      df = annotation_row[, ncol(annotation_row):1, drop = FALSE],
      col = annotation_colors,
      show_legend = .show_legend_left,
      show_annotation_name = annotation_names_row,
      gp = gpar(col = border_color),
      annotation_name_gp = gpar(fontsize = 10, fontface = "bold"),
      simple_anno_size = unit(10, "bigpts"),
      gap = unit(2, "bigpts"),
      annotation_legend_param = .anno_legend_param_left
    )
  }
  ...
  ht_param$row_names_gp = gpar(fontsize = fontsize_row, fontfamily = fontfamily_row,  
                               fontface = fontface_row)
  ht_param$column_names_gp = gpar(fontsize = fontsize_col,  
                                  fontfamily = fontfamily_col, fontface = fontface_col)
