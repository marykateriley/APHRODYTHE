coOncoplot_white <- function (m1, m2, genes = NULL, m1Name = NULL, m2Name = NULL, 
                              clinicalFeatures1 = NULL, clinicalFeatures2 = NULL, annotationColor1 = NULL, 
                              annotationColor2 = NULL, annotationFontSize = 1.2, sortByM1 = FALSE, 
                              sortByM2 = FALSE, sortByAnnotation1 = FALSE, annotationOrder1 = NULL, 
                              sortByAnnotation2 = FALSE, annotationOrder2 = NULL, sampleOrder1 = NULL, 
                              sampleOrder2 = NULL, additionalFeature1 = NULL, additionalFeaturePch1 = 20, 
                              additionalFeatureCol1 = "white", additionalFeatureCex1 = 0.9, 
                              additionalFeature2 = NULL, additionalFeaturePch2 = 20, additionalFeatureCol2 = "white", 
                              additionalFeatureCex2 = 0.9, sepwd_genes1 = 0.5, sepwd_samples1 = 0.5, 
                              sepwd_genes2 = 0.5, sepwd_samples2 = 0.5, colors = NULL, 
                              removeNonMutated = TRUE, anno_height = 2, legend_height = 4, 
                              geneNamefont = 0.8, showSampleNames = FALSE, SampleNamefont = 0.5, 
                              barcode_mar = 1, outer_mar = 3, gene_mar = 1, legendFontSize = 1.2, 
                              titleFontSize = 1.5, keepGeneOrder = FALSE, bgCol = "#ecf0f1", 
                              borderCol = "white") 
{
  if (is.null(genes)) {
    genes = unique(c(getGeneSummary(m1)[1:5, Hugo_Symbol], 
                     getGeneSummary(m2)[1:5, Hugo_Symbol]))
  }
  else {
  ...
  mdt = merge(m1.genes[, .(Hugo_Symbol, MutatedSamples)], m2.genes[, 
                                                                   .(Hugo_Symbol, MutatedSamples)], by = "Hugo_Symbol", 
              all = TRUE, suffixes = c("_m1", "_m2"))
  mdt$MutatedSamples_m1[is.na(mdt$MutatedSamples_m1)] = 0
  mdt$MutatedSamples_m2[is.na(mdt$MutatedSamples_m2)] = 0
  mdt$max = apply(mdt[, .(MutatedSamples_m1, MutatedSamples_m2)], 
                  1, max)
  if (sortByM1) {
    ...
    mdt = data.table::rbindlist(l = mdt, use.names = TRUE, 
                                fill = TRUE)
  }
  else {
  ...
  if (!is.null(clinicalFeatures1) & !is.null(clinicalFeatures2)) {
    if (is.null(annotationColor1) & is.null(annotationColor2)) {
      m12_annotation_colors = get_m12_annotation_colors(a1 = m1, 
                                                        a1_cf = clinicalFeatures1, a2 = m2, a2_cf = clinicalFeatures2)
      annotationColor1 = m12_annotation_colors
      annotationColor2 = m12_annotation_colors
    }
  }
  nm1 = maftools:::print_mat(maf = m1, genes = genes, removeNonMutated = removeNonMutated, 
                  test = TRUE, colors = colors)
  nm1_ncol = ncol(nm1[[1]])
  nm1_vc_cols = nm1[[2]]
  nm2 = maftools:::print_mat(maf = m2, genes = genes, removeNonMutated = removeNonMutated, 
                  test = TRUE, colors = colors)
  nm2_ncol = ncol(nm2[[1]])
  nm2_vc_cols = nm2[[2]]
  if (!is.null(clinicalFeatures1) || !is.null(clinicalFeatures2)) {
    mat_lo = matrix(data = c(1, 3, 5, 2, 4, 6, 7, 7, 7), 
                    nrow = 3, ncol = 3, byrow = TRUE)
    mat_lo = graphics::layout(mat = mat_lo, widths = c(6 * (nm1_ncol/nm2_ncol), gene_mar, 6), heights = c(12, 
                                                                                                          anno_height, legend_height))
  }
  else {
    mat_lo = matrix(data = c(1, 2, 3, 4, 4, 4), nrow = 2, 
                    ncol = 3, byrow = TRUE)
    mat_lo = graphics::layout(mat = mat_lo, widths = c(6 * (nm1_ncol/nm2_ncol), gene_mar, 6), heights = c(12, 
                                                                                                          legend_height))
  }
  m1_legend = maftools:::print_mat(maf = m1, genes = genes, removeNonMutated = removeNonMutated, 
                        clinicalFeatures = clinicalFeatures1, colors = colors, 
                        annotationColor = annotationColor1, barcode_size = SampleNamefont, 
                        sortByAnnotation = sortByAnnotation1, annotationOrder = annotationOrder1, 
                        fontSize = geneNamefont, title = m1Name, title_size = titleFontSize, 
                        ...
                        sepwd_genes = sepwd_genes1, sepwd_samples = sepwd_samples1, 
                        barcodemar = barcode_mar, genemar = outer_mar, sampleOrder = sampleOrder1)
  ...
  text(x = nrow(nm), y = 1:ncol(nm), labels = rev(genes), xpd = TRUE, 
       font = 3, cex = geneNamefont * 1.5)
  if (!is.null(clinicalFeatures1) || !is.null(clinicalFeatures2)) {
    plot.new()
  }
  m2_legend = maftools:::print_mat(maf = m2, genes = genes, removeNonMutated = removeNonMutated, 
                        clinicalFeatures = clinicalFeatures2, colors = colors, 
                        annotationColor = annotationColor2, barcode_size = SampleNamefont, 
                        sortByAnnotation = sortByAnnotation2, annotationOrder = annotationOrder2, 
                        fontSize = geneNamefont, title = m2Name, title_size = titleFontSize, 
                        plot2 = TRUE, showBarcodes = showSampleNames, bgCol = bgCol, 
                        borderCol = borderCol, additionalFeature = additionalFeature2, 
                        additionalFeaturePch = additionalFeaturePch2, additionalFeatureCex = additionalFeatureCex2, 
                        additionalFeatureCol = additionalFeatureCol2, sepwd_genes = sepwd_genes2, 
                        sepwd_samples = sepwd_samples2, barcodemar = barcode_mar, 
                        genemar = outer_mar, sampleOrder = sampleOrder2)
  ...
  if (!is.null(additionalFeature1)) {
    ...
    if (length(additionalFeaturePch1) != length(additionalFeature1)) {
      additionalFeaturePch1 = rep(additionalFeaturePch1, 
                                  length(additionalFeature1))
    }
    if (length(additionalFeatureCol1) != length(additionalFeature1)) {
      additionalFeatureCol1 = rep(additionalFeatureCol1, 
                                  length(additionalFeature1))
    }
  ...
  if (!is.null(additionalFeature2)) {
    ...
    if (length(additionalFeaturePch2) != length(additionalFeature2)) {
      ...
      additionalFeaturePch2 = rep(additionalFeaturePch2, 
                                  length(additionalFeature2))
    }
    if (length(additionalFeatureCol2) != length(additionalFeature2)) {
      warning("Provided colors for additional features are recycled")
      additionalFeatureCol2 = rep(additionalFeatureCol2, 
                                  length(additionalFeature2))
    }
  ...
  plot(NULL, ylab = "", xlab = "", xlim = 0:1, ylim = 0:1, 
       axes = FALSE)
  lep = legend("topleft", legend = names(vc_legend), col = vc_legend, 
               border = NA, bty = "n", ncol = 2, pch = vc_pch, xpd = TRUE, 
  ...
  if (!is.null(anno_legend)) {
    for (i in 1:length(anno_legend)) {
      ...
      lep = legend(x = x_axp, y = 1, legend = names(x), 
                   ...
                   title = names(anno_legend)[i], title.adj = 0)
